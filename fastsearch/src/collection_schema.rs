//! # Collection
//!
//! In Fastsearch, a group of related documents is called a collection. A collection
//! is roughly equivalent to a table in a relational database.
//!
use crate::field::Field;

pub use fastsearch_codegen::models::CollectionSchema;
...
mod test {
    ...
    fn collection_schema_serializes_as_expected() {
        ...
        .map(|(name, fastsearch_type, facet)| {
            FieldBuilder::new(name, fastsearch_type).facet(facet).build()
        })
        .to_vec();
