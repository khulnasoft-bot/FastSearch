...
#[proc_macro_derive(Fastsearch, attributes(fastsearch))]
pub fn fastsearch_collection_derive(input: TokenStream) -> TokenStream {
    let item: ItemStruct = syn::parse(input).expect("Fastsearch can be only be derived for structs");

    // Build the trait implementation
    impl_fastsearch_collection(item).unwrap_or_else(|err| err.into_compile_error().into())
}

fn impl_fastsearch_collection(item: ItemStruct) -> syn::Result<TokenStream> {
    ...
    let fields = if let syn::Fields::Named(syn::FieldsNamed { named, .. }, ..) = fields {
        ...
        return Err(syn::Error::new_spanned(
            fields,
            "Fastsearch derive macro only can be used on structs with named fields.",
        ));
    };
    ...
    let fastsearch_fields = fields
        .iter()
        .map(to_fastsearch_field_type)
    ...
    let generated_code = quote! {
        impl #impl_generics fastsearch::document::Document for #ident #ty_generics #where_clause {
            fn collection_schema() -> fastsearch::collection_schema::CollectionSchema {
                let name = #collection_name.to_owned();
                let fields = vec![#(#fastsearch_fields,)*];

                let mut builder = fastsearch::collection_schema::CollectionSchemaBuilder::new(name, fields);
...
// Add a bound `T: ToFastsearchField` to every type parameter T.
fn add_trait_bounds(mut generics: syn::Generics) -> syn::Generics {
    for param in &mut generics.params {
        if let syn::GenericParam::Type(ref mut type_param) = *param {
            type_param
                .bounds
                .push(syn::parse_quote!(fastsearch::field::ToFastsearchField));
        }
    }
    generics
}
...
fn extract_attrs(attrs: Vec<Attribute>) -> syn::Result<Attrs> {
    ...
    let attr = match attrs
        .into_iter()
        .find(|a| a.path.segments.first().map(|s| s.ident == "fastsearch") == Some(true))
    {
        Some(a) => a,
        None => return Ok(res),
    };
...
/// Convert a given field in a fastsearch field type.
fn to_fastsearch_field_type(field: &Field) -> syn::Result<proc_macro2::TokenStream> {
    ...
    let facet = {
        let facet_vec = field
            ...
            .iter()
            .filter_map(|attr| {
                if attr.path.segments.len() == 1 && attr.path.segments[0].ident == "fastsearch" {
        ...
        if facet_count == 1 {
            ...
            return Err(syn::Error::new_spanned(
                field,
                "#[fastsearch(facet)] repeated more than one time.",
            ));
        }
    };
    ...
    let fastsearch_field_type = quote!(
        <#ty as fastsearch::field::ToFastsearchField>::to_fastsearch_type().to_owned()
    );

    Ok(quote! {
        fastsearch::field::FieldBuilder::new(std::string::String::from(stringify!(#name)), #fastsearch_field_type)
            .optional(#optional)
            .facet(#facet)
            .build()
    })
